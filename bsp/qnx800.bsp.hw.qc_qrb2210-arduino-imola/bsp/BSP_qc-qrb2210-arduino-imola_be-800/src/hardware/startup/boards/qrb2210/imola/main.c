...
/*
 * STMicroelectronics QRB2210 Board with Cortex-A35 cores
 */
 ...
 *
 * @file       boards/qrb2210/dk/main.c
 * @addtogroup startup
 * @{
 */

#define QRB2210_WDT_ENABLE  (1 << 1)  /* watchdog enable */
...
/* The debug/console UART is the GENI (QUP) based uart4 serial engine */
struct debug_device debug_devices[] = {
    {
        .name = "qrb2210_uart4",
        /* GENI serial engine (uart4) base address: QRB2210_UART4_SE_BASE */
        .defaults[DEBUG_DEV_CONSOLE] = "0x4a90000^0.115200",
        .defaults[DEBUG_DEV_KDEBUG] = NULL,
        .init = qrb2210_init_uart,
        .put = qrb2210_uart_put_char,
        .callouts[DEBUG_DISPLAY_CHAR] = &qrb2210_uart_display_char,
        .callouts[DEBUG_POLL_KEY] = &qrb2210_uart_poll_key,
        .callouts[DEBUG_BREAK_DETECT] = &qrb2210_uart_break_detect,
    },
};
...
int main(const int argc, char **const argv, const char **const envv)
{
    ...
    /* TF-A of qrb2210 already providing watchdog and
     * it's interface to OS over SMCCC as arm,smc-wdt dts entity
     */
    // status = qrb2210_wdt_reset();
    // kprintf("Reset watchdog. Status: %d\n", status);
    // if ((options & QRB2210_WDT_ENABLE) != QRB2210_WDT_ENABLE) {
    //     status = qrb2210_wdt_stop();
    ...
    if (shdr->flags1 & STARTUP_HDR_FLAGS1_VIRTUAL) {
        ...
        qrb2210_board_mmu_enable(QRB2210_DRAM0_OS_BASE,
                                  QRB2210_DRAM0_OS_SIZE);
        // board_alignment_check_disable();
    }

    // qrb2210_init_pcie_ext_msi_controller();
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/startup/boards/qrb2210/dk/main.c $ $Rev: 985114 $")
#endif
